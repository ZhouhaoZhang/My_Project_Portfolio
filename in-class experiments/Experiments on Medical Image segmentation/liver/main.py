...
class Exp:
    def __init__(self):
        ...
        # 实例化模型
        self.module = UNet(1, 1).to(self.device)
        # 交叉熵损失
    ...
    def make_dataset(self):
        ...
        train_data = torch.zeros((len(files_img_train), 1, 512, 512))
        train_label = torch.zeros((len(files_lab_train), 1, 512, 512))
        test_data = torch.zeros((len(files_img_test), 1, 512, 512))
        test_label = torch.zeros((len(files_lab_test), 1, 512, 512))

        self.train_size = len(files_img_train)
        self.test_size = len(files_img_test)

        normalize = transforms.Normalize(mean=[59.9969], std=[59.3439])

        for file, i in zip(files_img_train, range(len(files_img_train))):
            train_data[i] = normalize(read_image(file).float())

        for file, i in zip(files_lab_train, range(len(files_lab_train))):
            train_label[i] = read_image(file).float() / 255

        for file, i in zip(files_img_test, range(len(files_img_test))):
            test_data[i] = normalize(read_image(file).float())

        for file, i in zip(files_lab_test, range(len(files_lab_test))):
            test_label[i] = read_image(file).float() / 255
