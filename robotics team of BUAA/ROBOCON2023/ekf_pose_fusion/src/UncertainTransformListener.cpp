#include "ekf_pose_fusion/UncertainTransformListener.h"
...
namespace uncertain_tf
{
    ...
    {
        message_subscriber_utf_ = node_.subscribe<ekf_pose_fusion::utfMessage>("/tf_uncertainty", 100, boost::bind(&UncertainTransformListener::subscription_callback, this, _1));
    ...
    void UncertainTransformListener::subscription_callback(const ekf_pose_fusion::utfMessageConstPtr &msg)
    {
        // std::cout << "got a message :" << endl;
        for (std::vector<ekf_pose_fusion::CovarianceStamped>::const_iterator it = msg->covariances.begin(); it != msg->covariances.end(); ++it)
        {
            ekf_pose_fusion::CovarianceStamped sc_msg = *it;
