...
#include "ekf_pose_fusion/EigenMultiVariateNormal.hpp"

// 功能构思
// 发布transform、cov，并附带时间戳
// cache tr cov 的时间序列
// 能获取所需时间内的tr以及cov
// 使用封闭类，并对主要功能加以封装
...
namespace uncertain_tf
{
    ...
    public:
        ...
        void calculateSampleCovariance(const Eigen::MatrixBase<Derived> &x, const MatrixBase<Derived> &y, MatrixBase<OtherDerived> &C_);
    ...
    private:
        void subscription_callback(const ekf_pose_fusion::utfMessageConstPtr &msg);
