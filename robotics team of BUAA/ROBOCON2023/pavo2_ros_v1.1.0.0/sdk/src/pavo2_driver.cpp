...
bool pavo2::pavo2_driver::get_scanned_data(pavo_response_scan_t * data_buffer,uint16_t & count,uint32_t& timestamp,uint16_t timeout)
{
	bool ret;
	ret = handle_packet_ptr_->recvData(data_buffer,count,timestamp,timeout);
	return ret;
}

bool pavo2::pavo2_driver::get_scanned_data(std::vector<pavo_response_scan_t>& vec_buff,uint32_t& timestamp,uint16_t timeout)
{
	bool ret;
	vec_buff.clear();
	ret = handle_packet_ptr_->recvData(vec_buff,timestamp,timeout);
	return ret;
}

bool pavo2::pavo2_driver::get_scanned_data(pavo_response_pcd_t * data_buffer,uint16_t & count,uint32_t& timestamp,uint16_t timeout)
{
	bool ret;
	uint32_t cnt;
	std::vector<pavo_response_scan_t> cn;
	ret = handle_packet_ptr_->recvData(cn,timestamp,timeout);
	if(ret)
	{
		for(cnt = 0; cnt < cn.size() && (cnt < count); cnt++)
		{
...
bool pavo2::pavo2_driver::get_scanned_data(std::vector<pavo_response_pcd_t>& vec_buff,uint32_t& timestamp,uint16_t timeout)
{
	bool ret;
	uint32_t cnt;
	vec_buff.clear();
...
bool pavo2::pavo2_driver::get_max_angle_range(uint32_t & val)
{
	bool ret;
	unsigned char send_cmd[8];
	unsigned char recv_cmd[8];
	PAVO2_CMD_INDEX cmd_type = CMD_ANGLE_RANGLE_ENABLE;
	uint16_t recv_len = sizeof(recv_cmd) / sizeof(unsigned char);
	memcpy(send_cmd,PAVO2_CMD[cmd_type],CMD_LEN);
	send_cmd[CMD_SET_GET_INDEX] = CMD_GET;
	//send the cmd
...
bool pavo2::pavo2_driver::set_motor_speed(uint8_t val)
{
	bool ret;
	unsigned char send_cmd[8];
...
bool pavo2::pavo2_driver::get_motor_speed_enable(uint32_t & val)
{
	bool ret;
	unsigned char send_cmd[8];
	unsigned char recv_cmd[8];
	PAVO2_CMD_INDEX cmd_type = CMD_MOTOR_SPEED_ENABLE;
	uint16_t recv_len = sizeof(recv_cmd) / sizeof(unsigned char);
	memcpy(send_cmd,PAVO2_CMD[cmd_type],CMD_LEN);
	send_cmd[CMD_SET_GET_INDEX] = CMD_GET;
	//send the cmd
...
bool pavo2::pavo2_driver::get_angle_resolution_enable(uint32_t & val)
{
	bool ret;
	unsigned char send_cmd[8];
	unsigned char recv_cmd[8];
	PAVO2_CMD_INDEX cmd_type = CMD_ANGLE_RESOLUTION_ENABLE;
	uint16_t recv_len = sizeof(recv_cmd) / sizeof(unsigned char);
	memcpy(send_cmd,PAVO2_CMD[cmd_type],CMD_LEN);
	send_cmd[CMD_SET_GET_INDEX] = CMD_GET;
	//send the cmd
...
bool pavo2::pavo2_driver::enable_hr_filter(uint16_t hrLevel)
{
    return handle_packet_ptr_->enable_hr_filter(hrLevel);
}
...
bool pavo2::pavo2_driver::set_outdoor_mode(bool en)
{
	bool ret;
	unsigned char send_cmd[8];
	PAVO2_CMD_INDEX cmd_type = CMD_OUTDOOR;
	uint16_t send_len = sizeof(send_cmd) / sizeof(unsigned char);;
	memset(send_cmd,0,send_len);
	memcpy(send_cmd,PAVO2_CMD[cmd_type],CMD_LEN);
	send_cmd[CMD_SET_GET_INDEX] = CMD_SET;
...
bool pavo2::pavo2_driver::get_outdoor_mode(bool& en)
{
	bool ret;
	unsigned char send_cmd[8];
	unsigned char recv_cmd[8];
...
bool pavo2::pavo2_driver::set_calibrate_horizontal_angle(uint32_t angle)
{
	if(angle < 0 && angle >= 36000) return false;

	bool ret;
	unsigned char send_cmd[8];
	PAVO2_CMD_INDEX cmd_type = CMD_CAL_HOR_ANGLE;
	uint16_t send_len = sizeof(send_cmd) / sizeof(unsigned char);
	memset(send_cmd,0,send_len);
	//set the content
	memcpy(send_cmd,PAVO2_CMD[cmd_type],CMD_LEN);
	...
	send_cmd[CMD_LEN] = 0xFF & (angle >> 24);
	send_cmd[CMD_LEN + 1] = 0xFF & (angle >> 16);
	send_cmd[CMD_LEN + 2] = 0xFF & (angle >> 8);
	send_cmd[CMD_LEN + 3] = 0xFF & (angle);
	//set the send len
	send_len = CMD_LEN + PAVO2_CMD_ARG_LEN[cmd_type];
	//send the cmd
	ret = handle_packet_ptr_->sendCMD(send_cmd,send_len);
...
bool pavo2::pavo2_driver::get_calibrate_horizontal_angle(uint32_t& angle)
{
	bool ret;
	unsigned char send_cmd[8];
	unsigned char recv_cmd[8];
	...
	if(!ret) return false;
	//check the value
	angle = ((uint32_t)recv_cmd[CMD_LEN + 3]) + (((uint32_t)recv_cmd[CMD_LEN + 2]) << 8) + (((uint32_t)recv_cmd[CMD_LEN + 1]) << 16) + (((uint32_t)recv_cmd[CMD_LEN]) << 24);
	return ret;
}

bool pavo2::pavo2_driver::get_max_angle_distance(uint32_t & val)
{
	bool ret;
	unsigned char send_cmd[8];
	unsigned char recv_cmd[8];
	PAVO2_CMD_INDEX cmd_type = CMD_MAX_DISTANCE;
	uint16_t recv_len = sizeof(recv_cmd) / sizeof(unsigned char);
	memcpy(send_cmd,PAVO2_CMD[cmd_type],CMD_LEN);
	send_cmd[CMD_SET_GET_INDEX] = CMD_GET;
	//send the cmd
